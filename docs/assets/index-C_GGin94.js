...
import{r as s,j as e,L as ae,u as at,c as rt,d as st,e as it,f as V,H as lt,g as ot}from"./react-vendor-6zld6QO8.js";import{A as qe,m as Se}from"./framer-DvVmTyA1.js";import{M as dt,r as ct,a as ut}from"./markdown-q3GborES.js";import{b as ze}from"./geo-Z02C8O4I.js";import{m as Ce}from"./maplibre-BedwuVXB.js";import{U as Ge,L as mt,D as ht,R as gt,M as pt,C as xt,a as ft,b as bt,X as Ue,c as Fe}from"./icons-Dw2J7OpA.js";import"./vendor-C2tZ253J.js";(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const b of document.querySelectorAll('link[rel="modulepreload"]'))o(b);new MutationObserver(b=>{for(const c of b)if(c.type==="childList")for(const n of c.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&o(n)}).observe(document,{childList:!0,subtree:!0});function x(b){const c={};return b.integrity&&(c.integrity=b.integrity),b.referrerPolicy&&(c.referrerPolicy=b.referrerPolicy),b.crossOrigin==="use-credentials"?c.credentials="include":b.crossOrigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function o(b){if(b.ep)return;b.ep=!0;const c=x(b);fetch(b.href,c)}})();function vt({onFinish:i}){const[r,x]=s.useState(!0),o=s.useRef(null),b=s.useRef({x:-1e3,y:-1e3});return s.useEffect(()=>{const c=o.current;if(!c)return;const n=c.getContext("2d");if(!n)return;let t,l=[];const N=()=>{c.width=window.innerWidth,c.height=window.innerHeight},w=u=>{b.current={x:u.clientX,y:u.clientY}},z=u=>{u.touches.length>0&&(b.current={x:u.touches[0].clientX,y:u.touches[0].clientY})};window.addEventListener("resize",N),window.addEventListener("mousemove",w),window.addEventListener("touchmove",z),N();class I{constructor(){this.x=Math.random()*c.width,this.y=Math.random()*c.height,this.baseX=this.x,this.baseY=this.y,this.vx=(Math.random()-.5)*.5,this.vy=(Math.random()-.5)*.5,this.size=Math.random()*2+.5,this.density=Math.random()*30+1}update(){this.x+=this.vx,this.y+=this.vy,(this.x<0||this.x>c.width)&&(this.vx*=-1),(this.y<0||this.y>c.height)&&(this.vy*=-1);let G=b.current.x-this.x,U=b.current.y-this.y,R=Math.sqrt(G*G+U*U),E=G/R,_=U/R;const $=150;let ee=($-R)/$;R<$&&(this.x-=E*ee*this.density*.5,this.y-=_*ee*this.density*.5)}draw(){n&&(n.fillStyle="rgba(255, 255, 255, 0.8)",n.beginPath(),n.arc(this.x,this.y,this.size,0,Math.PI*2),n.fill())}}for(let u=0;u<120;u++)l.push(new I);const F=()=>{n.clearRect(0,0,c.width,c.height);for(let u=0;u<l.length;u++){l[u].update(),l[u].draw();for(let G=u+1;G<l.length;G++){const U=l[u].x-l[G].x,R=l[u].y-l[G].y,E=Math.sqrt(U*U+R*R);E<100&&(n.beginPath(),n.strokeStyle=`rgba(255, 255, 255, ${.2*(1-E/100)})`,n.lineWidth=.6,n.moveTo(l[u].x,l[u].y),n.lineTo(l[G].x,l[G].y),n.stroke())}}t=requestAnimationFrame(F)};return F(),()=>{cancelAnimationFrame(t),window.removeEventListener("resize",N),window.removeEventListener("mousemove",w),window.removeEventListener("touchmove",z)}},[]),e.jsx(qe,{onExitComplete:i,children:r&&e.jsxs(Se.div,{onClick:()=>x(!1),initial:{opacity:1},exit:{opacity:0,filter:"blur(30px)",transition:{duration:1.2,ease:[.43,.13,.23,.96]}},className:"fixed inset-0 z-50 bg-black cursor-pointer overflow-hidden flex flex-col items-center justify-center",children:[e.jsx("canvas",{ref:o,className:"absolute inset-0 w-full h-full "}),e.jsx("div",{className:"relative z-20 text-center  select-none",children:e.jsx(Se.p,{initial:{opacity:0,y:10},animate:{opacity:.7,y:0},transition:{duration:2},className:"text-white font-extralight text-lg md:text-sm uppercase tracking-[0.8em]",children:"LILI LIN's Space 林丽丽"})}),e.jsx(Se.div,{initial:{opacity:0},animate:{opacity:.4},className:"absolute bottom-12 text-[10px] text-white font-light tracking-[0.5em] uppercase",children:"Touch the stars to enter"})]},"intro-screen")})}const yt=`---
id: "01"
...
拼拼将迎合以上⽤户的痛点， 以拼单事件为载体， 提供私聊功能， 为⽤户提供⼀个安全⾼效、 能双向选择、 信息对等的平台， 提升同学们的”拼单”效率， 便捷、 丰富同学们的⽣活。<br>`,St=`---
id: "02"
slug: "perslearn"
...
title: "2022 Tencent Games × HUST 72-Hour Game Jam"\r
category: "Game"\r
...
## 项目背景\r
\r
2022 年正值疫情时期，许多人因居家隔离无法外出，而一批默默无闻的配送人员和志愿者承担起了保障居民生活物资供应的重要职责。相比聚焦疫情本身，我们更希望通过游戏向这些为城市正常运转默默付出的“无名英雄”致敬，让玩家体验他们在配送过程中的责任与挑战。\r
\r
## 项目时间\r
\r
...
> 💡 **小提示**：MCP 是一个快速发展的开放标准，未来会有越来越多的工具和应用支持它。现在学习 MCP，就是提前掌握 AI 时代的「通用语言」！`,Qt=`---\r
id: "09"\r
