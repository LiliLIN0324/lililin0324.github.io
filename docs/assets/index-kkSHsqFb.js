...
import{r,j as e,L as te,u as mn,R as hn,c as fn,d as xn,e as bn,f as O,H as yn,g as jn}from"./react-vendor-6zld6QO8.js";import{A as vn,m as qe}from"./framer-DvVmTyA1.js";import{M as wn,r as Nn,a as kn}from"./markdown-q3GborES.js";import{b as Ye}from"./geo-Z02C8O4I.js";import{m as Re}from"./maplibre-BedwuVXB.js";import{U as Qe,L as Dn,D as An,R as Mn,M as _n,C as Sn,a as Cn,b as In,X as Je,c as en}from"./icons-Dw2J7OpA.js";import"./vendor-C2tZ253J.js";(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const d of document.querySelectorAll('link[rel="modulepreload"]'))s(d);new MutationObserver(d=>{for(const x of d)if(x.type==="childList")for(const b of x.addedNodes)b.tagName==="LINK"&&b.rel==="modulepreload"&&s(b)}).observe(document,{childList:!0,subtree:!0});function p(d){const x={};return d.integrity&&(x.integrity=d.integrity),d.referrerPolicy&&(x.referrerPolicy=d.referrerPolicy),d.crossOrigin==="use-credentials"?x.credentials="include":d.crossOrigin==="anonymous"?x.credentials="omit":x.credentials="same-origin",x}function s(d){if(d.ep)return;d.ep=!0;const x=p(d);fetch(d.href,x)}})();function Ln({onFinish:n}){const[a,p]=r.useState(!0),s=r.useRef(null),d=r.useRef({x:-1e3,y:-1e3});return r.useEffect(()=>{const x=s.current;if(!x)return;const b=x.getContext("2d");if(!b)return;let f,j=[];const C=()=>{x.width=window.innerWidth,x.height=window.innerHeight},D=u=>{d.current={x:u.clientX,y:u.clientY}},k=u=>{u.touches.length>0&&(d.current={x:u.touches[0].clientX,y:u.touches[0].clientY})};window.addEventListener("resize",C),window.addEventListener("mousemove",D),window.addEventListener("touchmove",k),C();class A{constructor(){this.x=Math.random()*x.width,this.y=Math.random()*x.height,this.baseX=this.x,this.baseY=this.y,this.vx=(Math.random()-.5)*.5,this.vy=(Math.random()-.5)*.5,this.size=Math.random()*2+.5,this.density=Math.random()*30+1}update(){this.x+=this.vx,this.y+=this.vy,(this.x<0||this.x>x.width)&&(this.vx*=-1),(this.y<0||this.y>x.height)&&(this.vy*=-1);let R=d.current.x-this.x,G=d.current.y-this.y,F=Math.sqrt(R*R+G*G),E=R/F,U=G/F;const B=150;let _=(B-F)/B;F<B&&(this.x-=E*_*this.density*.5,this.y-=U*_*this.density*.5)}draw(){b&&(b.fillStyle="rgba(255, 255, 255, 0.8)",b.beginPath(),b.arc(this.x,this.y,this.size,0,Math.PI*2),b.fill())}}for(let u=0;u<120;u++)j.push(new A);const P=()=>{b.clearRect(0,0,x.width,x.height);for(let u=0;u<j.length;u++){j[u].update(),j[u].draw();for(let R=u+1;R<j.length;R++){const G=j[u].x-j[R].x,F=j[u].y-j[R].y,E=Math.sqrt(G*G+F*F);E<100&&(b.beginPath(),b.strokeStyle=`rgba(255, 255, 255, ${.2*(1-E/100)})`,b.lineWidth=.6,b.moveTo(j[u].x,j[u].y),b.lineTo(j[R].x,j[R].y),b.stroke())}}f=requestAnimationFrame(P)};return P(),()=>{cancelAnimationFrame(f),window.removeEventListener("resize",C),window.removeEventListener("mousemove",D),window.removeEventListener("touchmove",k)}},[]),e.jsx(vn,{onExitComplete:n,children:a&&e.jsxs(qe.div,{onClick:()=>p(!1),initial:{opacity:1},exit:{opacity:0,filter:"blur(30px)",transition:{duration:1.2,ease:[.43,.13,.23,.96]}},className:"fixed inset-0 z-50 bg-black cursor-pointer overflow-hidden flex flex-col items-center justify-center",children:[e.jsx("canvas",{ref:s,className:"absolute inset-0 w-full h-full "}),e.jsx("div",{className:"relative z-20 text-center  select-none",children:e.jsx(qe.p,{initial:{opacity:0,y:10},animate:{opacity:.7,y:0},transition:{duration:2},className:"text-white font-extralight text-lg md:text-sm uppercase tracking-[0.8em]",children:"LILI LIN's Space 林丽丽"})}),e.jsx(qe.div,{initial:{opacity:0},animate:{opacity:.4},className:"absolute bottom-12 text-[10px] text-white font-light tracking-[0.5em] uppercase",children:"Touch the stars to enter"})]},"intro-screen")})}const Pn=`---
id: "00"
slug: "chicken-utopia"
