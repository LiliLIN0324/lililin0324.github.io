...
import{r as n,j as e,L as be,u as oc,R as hc,c as lc,d as vc,e as mc,f as F,H as jc,g as yc}from"./react-vendor-6zld6QO8.js";import{A as xc,m as $e}from"./framer-DvVmTyA1.js";import{M as wc,r as kc,a as zc}from"./markdown-q3GborES.js";import{b as Ye}from"./geo-Z02C8O4I.js";import{m as Ee}from"./maplibre-BedwuVXB.js";import{U as Qe,L as qc,D as Nc,R as Mc,M as Sc,C as Ic,a as Cc,b as Ac,X as Je,c as ec}from"./icons-Dw2J7OpA.js";import"./vendor-C2tZ253J.js";(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))b(r);new MutationObserver(r=>{for(const l of r)if(l.type==="childList")for(const g of l.addedNodes)g.tagName==="LINK"&&g.rel==="modulepreload"&&b(g)}).observe(document,{childList:!0,subtree:!0});function u(r){const l={};return r.integrity&&(l.integrity=r.integrity),r.referrerPolicy&&(l.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?l.credentials="include":r.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function b(r){if(r.ep)return;r.ep=!0;const l=u(r);fetch(r.href,l)}})();function Dc({onFinish:c}){const[a,u]=n.useState(!0),b=n.useRef(null),r=n.useRef({x:-1e3,y:-1e3});return n.useEffect(()=>{const l=b.current;if(!l)return;const g=l.getContext("2d");if(!g)return;let C,w=[];const P=()=>{l.width=window.innerWidth,l.height=window.innerHeight},q=f=>{r.current={x:f.clientX,y:f.clientY}},j=f=>{f.touches.length>0&&(r.current={x:f.touches[0].clientX,y:f.touches[0].clientY})};window.addEventListener("resize",P),window.addEventListener("mousemove",q),window.addEventListener("touchmove",j),P();class k{constructor(){this.x=Math.random()*l.width,this.y=Math.random()*l.height,this.baseX=this.x,this.baseY=this.y,this.vx=(Math.random()-.5)*.5,this.vy=(Math.random()-.5)*.5,this.size=Math.random()*2+.5,this.density=Math.random()*30+1}update(){this.x+=this.vx,this.y+=this.vy,(this.x<0||this.x>l.width)&&(this.vx*=-1),(this.y<0||this.y>l.height)&&(this.vy*=-1);let E=r.current.x-this.x,G=r.current.y-this.y,B=Math.sqrt(E*E+G*G),L=E/B,J=G/B;const T=150;let M=(T-B)/T;B<T&&(this.x-=L*M*this.density*.5,this.y-=J*M*this.density*.5)}draw(){g&&(g.fillStyle="rgba(255, 255, 255, 0.8)",g.beginPath(),g.arc(this.x,this.y,this.size,0,Math.PI*2),g.fill())}}for(let f=0;f<120;f++)w.push(new k);const N=()=>{g.clearRect(0,0,l.width,l.height);for(let f=0;f<w.length;f++){w[f].update(),w[f].draw();for(let E=f+1;E<w.length;E++){const G=w[f].x-w[E].x,B=w[f].y-w[E].y,L=Math.sqrt(G*G+B*B);L<100&&(g.beginPath(),g.strokeStyle=`rgba(255, 255, 255, ${.2*(1-L/100)})`,g.lineWidth=.6,g.moveTo(w[f].x,w[f].y),g.lineTo(w[E].x,w[E].y),g.stroke())}}C=requestAnimationFrame(N)};return N(),()=>{cancelAnimationFrame(C),window.removeEventListener("resize",P),window.removeEventListener("mousemove",q),window.removeEventListener("touchmove",j)}},[]),e.jsx(xc,{onExitComplete:c,children:a&&e.jsxs($e.div,{onClick:()=>u(!1),initial:{opacity:1},exit:{opacity:0,filter:"blur(30px)",transition:{duration:1.2,ease:[.43,.13,.23,.96]}},className:"fixed inset-0 z-50 bg-black cursor-pointer overflow-hidden flex flex-col items-center justify-center",children:[e.jsx("canvas",{ref:b,className:"absolute inset-0 w-full h-full "}),e.jsx("div",{className:"relative z-20 text-center  select-none",children:e.jsx($e.p,{initial:{opacity:0,y:10},animate:{opacity:.7,y:0},transition:{duration:2},className:"text-white font-extralight text-lg md:text-sm uppercase tracking-[0.8em]",children:"LILI LIN's Space 林丽丽"})}),e.jsx($e.div,{initial:{opacity:0},animate:{opacity:.4},className:"absolute bottom-12 text-[10px] text-white font-light tracking-[0.5em] uppercase",children:"Touch the stars to enter"})]},"intro-screen")})}const Lc=`---
id: "00"
slug: "chicken-utopia"
...
https://learn.u3d.cn/tutorial/instant-game-joker?chapterId=64f5962ca571d50021bb3afb&sectionId=64f5962ca571d50021bb3afa`,tb=`---\r
id: "01"\r
slug: "litflow"\r
...
> 💡 **小提示**：没有绝对最好的数据库，只有最适合你项目的数据库。根据实际需求选择，才是最明智的决定！`;function hb(c){const a=/^---\r?\n([\s\S]*?)\r?\n---/,u=c.match(a);let b={},r=c;if(u){const g=u[1];r=c.slice(u[0].length).trim();const C=g.split(/\r?\n/);let w=null,P=[];C.forEach(q=>{if(q.trim()==="")return;const k=q.indexOf(":");if(k!==-1&&!q.startsWith(" ")){w&&(b[w]=P.join(`
`)),w=q.slice(0,k).trim();let N=q.slice(k+1).trim();N==="|"?P=[]:N===""?P=[]:(b[w]=N,w=null,P=[])}else w&&q.startsWith("  ")&&P.push(q.slice(2))}),w&&P.length>0&&(b[w]=P.join(`
