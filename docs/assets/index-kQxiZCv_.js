...
import{r as n,j as e,L as be,u as oc,R as hc,c as lc,d as vc,e as mc,f as F,H as jc,g as yc}from"./react-vendor-6zld6QO8.js";import{A as xc,m as $e}from"./framer-DvVmTyA1.js";import{M as wc,r as kc,a as zc}from"./markdown-q3GborES.js";import{b as Ye}from"./geo-Z02C8O4I.js";import{m as Pe}from"./maplibre-BedwuVXB.js";import{U as Qe,L as qc,D as Nc,R as Mc,M as Sc,C as Cc,a as Ic,b as Ac,X as Je,c as ec}from"./icons-Dw2J7OpA.js";import"./vendor-C2tZ253J.js";(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))b(i);new MutationObserver(i=>{for(const l of i)if(l.type==="childList")for(const g of l.addedNodes)g.tagName==="LINK"&&g.rel==="modulepreload"&&b(g)}).observe(document,{childList:!0,subtree:!0});function u(i){const l={};return i.integrity&&(l.integrity=i.integrity),i.referrerPolicy&&(l.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?l.credentials="include":i.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function b(i){if(i.ep)return;i.ep=!0;const l=u(i);fetch(i.href,l)}})();function Dc({onFinish:c}){const[a,u]=n.useState(!0),b=n.useRef(null),i=n.useRef({x:-1e3,y:-1e3});return n.useEffect(()=>{const l=b.current;if(!l)return;const g=l.getContext("2d");if(!g)return;let I,w=[];const E=()=>{l.width=window.innerWidth,l.height=window.innerHeight},q=f=>{i.current={x:f.clientX,y:f.clientY}},j=f=>{f.touches.length>0&&(i.current={x:f.touches[0].clientX,y:f.touches[0].clientY})};window.addEventListener("resize",E),window.addEventListener("mousemove",q),window.addEventListener("touchmove",j),E();class k{constructor(){this.x=Math.random()*l.width,this.y=Math.random()*l.height,this.baseX=this.x,this.baseY=this.y,this.vx=(Math.random()-.5)*.5,this.vy=(Math.random()-.5)*.5,this.size=Math.random()*2+.5,this.density=Math.random()*30+1}update(){this.x+=this.vx,this.y+=this.vy,(this.x<0||this.x>l.width)&&(this.vx*=-1),(this.y<0||this.y>l.height)&&(this.vy*=-1);let P=i.current.x-this.x,G=i.current.y-this.y,B=Math.sqrt(P*P+G*G),L=P/B,J=G/B;const T=150;let M=(T-B)/T;B<T&&(this.x-=L*M*this.density*.5,this.y-=J*M*this.density*.5)}draw(){g&&(g.fillStyle="rgba(255, 255, 255, 0.8)",g.beginPath(),g.arc(this.x,this.y,this.size,0,Math.PI*2),g.fill())}}for(let f=0;f<120;f++)w.push(new k);const N=()=>{g.clearRect(0,0,l.width,l.height);for(let f=0;f<w.length;f++){w[f].update(),w[f].draw();for(let P=f+1;P<w.length;P++){const G=w[f].x-w[P].x,B=w[f].y-w[P].y,L=Math.sqrt(G*G+B*B);L<100&&(g.beginPath(),g.strokeStyle=`rgba(255, 255, 255, ${.2*(1-L/100)})`,g.lineWidth=.6,g.moveTo(w[f].x,w[f].y),g.lineTo(w[P].x,w[P].y),g.stroke())}}I=requestAnimationFrame(N)};return N(),()=>{cancelAnimationFrame(I),window.removeEventListener("resize",E),window.removeEventListener("mousemove",q),window.removeEventListener("touchmove",j)}},[]),e.jsx(xc,{onExitComplete:c,children:a&&e.jsxs($e.div,{onClick:()=>u(!1),initial:{opacity:1},exit:{opacity:0,filter:"blur(30px)",transition:{duration:1.2,ease:[.43,.13,.23,.96]}},className:"fixed inset-0 z-50 bg-black cursor-pointer overflow-hidden flex flex-col items-center justify-center",children:[e.jsx("canvas",{ref:b,className:"absolute inset-0 w-full h-full "}),e.jsx("div",{className:"relative z-20 text-center  select-none",children:e.jsx($e.p,{initial:{opacity:0,y:10},animate:{opacity:.7,y:0},transition:{duration:2},className:"text-white font-extralight text-lg md:text-sm uppercase tracking-[0.8em]",children:"LILI LIN's Space 林丽丽"})}),e.jsx($e.div,{initial:{opacity:0},animate:{opacity:.4},className:"absolute bottom-12 text-[10px] text-white font-light tracking-[0.5em] uppercase",children:"Touch the stars to enter"})]},"intro-screen")})}const Lc=`---
id: "00"
slug: "chicken-utopia"
...
![Genshot99](https://pub-3209bcb7fc36444a914deb0e70ceca92.r2.dev/fig/genshot/99.jpg)`,$c=`---
id: "04"
slug: "genstyle"
...
![genstyle](https://pub-3209bcb7fc36444a914deb0e70ceca92.r2.dev/fig/genstyle/99.jpg)`,Kc=`---
id: "05"
...
同时，我也更加明确了自己的优势与不足：空间设计能力能够迁移到游戏场景中，而构图、色彩和美术表达仍需要持续积累。`,cb=`---
id: "05"
...
https://learn.u3d.cn/tutorial/instant-game-joker?chapterId=64f5962ca571d50021bb3afb&sectionId=64f5962ca571d50021bb3afa`,tb=`---
id: "01"
slug: "litflow"
...
> 💡 **小提示**：MCP 是一个快速发展的开放标准，未来会有越来越多的工具和应用支持它。现在学习 MCP，就是提前掌握 AI 时代的「通用语言」！`,ob=`---
id: "09"
...
> 💡 **小提示**：没有绝对最好的数据库，只有最适合你项目的数据库。根据实际需求选择，才是最明智的决定！`;function hb(c){const a=/^---\r?\n([\s\S]*?)\r?\n---/,u=c.match(a);let b={},i=c;if(u){const g=u[1];i=c.slice(u[0].length).trim();const I=g.split(/\r?\n/);let w=null,E=[];I.forEach(q=>{if(q.trim()==="")return;const k=q.indexOf(":");if(k!==-1&&!q.startsWith(" ")){w&&(b[w]=E.join(`
...
`)&&/^\s*-/.test(j)){const k=j.split(/\r?\n/).map(N=>N.replace(/^\s*-\s*/,"").trim()).filter(N=>N!=="");b[q]=k.map(N=>N.startsWith('"')&&N.endsWith('"')||N.startsWith("'")&&N.endsWith("'")?N.slice(1,-1):N)}else if(j.startsWith("[")&&j.endsWith("]"))try{b[q]=JSON.parse(j)}catch{b[q]=[j.slice(1,-1).trim()]}else j==="true"?b[q]=!0:j==="false"?b[q]=!1:!isNaN(Number(j))&&j.trim()!==""?b[q]=Number(j):b[q]=j})}let l=[];if(Array.isArray(b.image)&&b.image.length>0)l=b.image;else{const g=/!\[.*?\]\(([^)\s]+)\)/g;let I;for(;(I=g.exec(i))!==null;)l.push(I[1])}return{id:b.id||"",slug:b.slug||"",title:b.title||"",category:b.category||"",year:b.year||"",description:b.description||"",tech:b.tech||[],abstract:b.abstract||"",challenge:b.challenge||"",solution:b.solution||"",hasDemo:b.hasDemo||!1,icon:b.icon||"",code:b.code||"",logo:b.icon||"",image:l,content:i}}function A(c){const a=hb(c);return{id:a.id,slug:a.slug,title:a.title,category:a.category,year:a.year,description:a.description,tech:a.tech,hasDemo:a.hasDemo,details:{abstract:a.abstract,description:a.description,challenge:a.challenge,solution:a.solution,logo:a.logo,icon:a.icon,code:a.code,image:a.image,content:a.content,demoOnly:a.demoOnly||!1}}}const Ee=[A(Lc),A(Pc),A(Ec),A(Tc),A(Oc),A(_c),A(Gc),A(Bc),A(Fc),A(Rc),A(Uc)],Re=[A(Wc),A(Vc),A(Hc),A($c),A(Kc),A(Xc),A(Zc)],Te=[A(Yc),A(Qc),A(Jc),A(eb),A(cb),A(bb)],_e=[A(tb),A(ab),A(pb)],Ge=[A(nb),A(ib),A(rb),A(gb),A(db),A(sb),A(fb),A(ub),A(ob)],lb={design:"Products",game:"Games",planning:"Planning",platform:"Platform",tutorial:"Tutorials",research:"Research"},sc=({className:c})=>e.jsx("div",{className:"flex h-full w-full items-center justify-center text-ink-3/50",children:e.jsx("svg",{className:c,fill:"none",stroke:"currentColor",strokeWidth:1.4,viewBox:"0 0 24 24","aria-hidden":"true",children:e.jsx("path",{strokeLinecap:"round",strokeLinejoin:"round",d:"M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"})})}),fc=({to:c,className:a=""})=>e.jsx(be,{to:c,className:`btn-accent ${a}`,children:"Demo"}),vb=!1,mb=({project:c,type:a,index:u})=>{var b;return e.jsx("li",{className:"group border-b border-rule transition-colors duration-300 hover:bg-surface",children:e.jsxs("div",{className:"flex items-center gap-4 py-4 md:gap-6 md:py-5",children:[e.jsx("span",{className:"nums-tabular hidden w-10 shrink-0 self-start pt-1 text-lg font-bold leading-none tracking-masthead text-ink-3/60 transition-colors group-hover:text-accent-text sm:block",children:String(u+1).padStart(2,"0")}),e.jsxs(be,{to:`/${a}/${c.slug}`,className:"flex min-w-0 flex-1 items-center gap-4 md:gap-6",children:[e.jsx("div",{className:"h-16 w-16 shrink-0 overflow-hidden border border-rule bg-surface-2 md:h-20 md:w-20",children:c.details.logo?e.jsx("img",{src:c.details.logo,alt:"",className:"h-full w-full object-cover transition-transform duration-700 ease-editorial group-hover:scale-105",onError:i=>{i.target.style.display="none"}}):e.jsx(sc,{className:"h-7 w-7"})}),e.jsxs("div",{className:"min-w-0 flex-1",children:[e.jsx("h3",{className:"truncate text-base font-bold tracking-tight text-ink md:text-lg lg:text-xl",children:e.jsx("span",{className:"wipe-underline",children:c.title})}),e.jsx("p",{className:"mt-1 truncate text-sm text-ink-2",children:c.description})]}),e.jsx("div",{className:"hidden shrink-0 flex-wrap justify-end gap-1.5 lg:flex lg:max-w-[16rem]",children:(b=c.tech)==null?void 0:b.slice(0,3).map(i=>e.jsx("span",{className:"chip",children:i},i))}),e.jsx("span",{className:"eyebrow nums-tabular hidden w-12 shrink-0 text-right md:block",children:c.year})]}),c.hasDemo&&e.jsx(fc,{to:`/${a}/${c.slug}#demo`,className:"shrink-0"})]})})},jb=({project:c,type:a,index:u})=>{var b;return e.jsxs("article",{className:"group flex flex-col border border-rule bg-surface transition-colors duration-300 hover:border-rule-strong",children:[e.jsxs(be,{to:`/${a}/${c.slug}`,className:"flex flex-1 flex-col",children:[e.jsx("div",{className:"aspect-square w-full overflow-hidden border-b border-rule bg-surface-2",children:c.details.logo?e.jsx("img",{src:c.details.logo,alt:"",className:"h-full w-full object-cover transition-transform duration-700 ease-editorial group-hover:scale-[1.05]",onError:i=>{i.target.style.display="none"}}):e.jsx(sc,{className:"h-10 w-10"})}),e.jsxs("div",{className:"flex flex-1 flex-col p-4",children:[e.jsxs("div",{className:"flex items-baseline justify-between gap-3",children:[e.jsx("span",{className:"nums-tabular text-sm font-bold tracking-masthead text-ink-3/70 transition-colors group-hover:text-accent-text",children:String(u+1).padStart(2,"0")}),e.jsx("span",{className:"eyebrow nums-tabular",children:c.year})]}),e.jsx("h3",{className:"mt-2.5 line-clamp-2 text-base font-bold leading-snug tracking-tight text-ink md:text-lg",children:e.jsx("span",{className:"wipe-underline",children:c.title})}),e.jsx("p",{className:"mt-1.5 line-clamp-2 text-sm leading-relaxed text-ink-2",children:c.description}),((b=c.tech)==null?void 0:b.length)>0&&e.jsx("div",{className:"mt-3 flex flex-wrap gap-1.5",children:c.tech.slice(0,2).map(i=>e.jsx("span",{className:"chip",children:i},i))})]})]}),c.hasDemo&&e.jsx("div",{className:"border-t border-rule p-3",children:e.jsx(fc,{to:`/${a}/${c.slug}#demo`,className:"w-full"})})]})},Se=({data:c,type:a})=>{const[u,b]=n.useState("grid"),i=lb[a]??"Works";return e.jsxs("div",{className:"shell animate-rise-in py-8 md:py-12",children:[a==="design"&&vb,e.jsxs("div",{className:"section-head",children:[e.jsxs("p",{className:"eyebrow",children:[a," / Index"]}),e.jsxs("p",{className:"eyebrow nums-tabular",children:[String(c.length).padStart(2,"0")," Entries"]})]}),e.jsxs("div",{className:"mt-5 flex flex-wrap items-end justify-between gap-4 border-b border-rule pb-5",children:[e.jsxs("h1",{className:"text-display-sm",children:["Selected ",i]}),e.jsx("div",{className:"flex shrink-0 border border-rule",role:"group","aria-label":"View mode",children:["grid","list"].map(l=>e.jsx("button",{onClick:()=>b(l),"aria-pressed":u===l,className:`px-3 py-1.5 font-mono text-[10px] uppercase tracking-eyebrow transition-colors duration-200 ${u===l?"bg-ink text-canvas":"text-ink-3 hover:text-ink"}`,children:l},l))})]}),u==="list"?e.jsx("ul",{className:"mt-2 border-t border-rule",children:c.map((l,g)=>e.jsx(mb,{project:l,type:a,index:g},l.slug))}):e.jsx("div",{className:"mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-5 lg:grid-cols-3 xl:grid-cols-4",children:c.map((l,g)=>e.jsx(jb,{project:l,type:a,index:g},l.slug))})]})},yb="modulepreload",xb=function(c,a){return new URL(c,a).href},cc={},_=function(a,u,b){let i=Promise.resolve();if(u&&u.length>0){let g=function(q){return Promise.all(q.map(j=>Promise.resolve(j).then(k=>({status:"fulfilled",value:k}),k=>({status:"rejected",reason:k}))))};const I=document.getElementsByTagName("link"),w=document.querySelector("meta[property=csp-nonce]"),E=(w==null?void 0:w.nonce)||(w==null?void 0:w.getAttribute("nonce"));i=g(u.map(q=>{if(q=xb(q,b),q in cc)return;cc[q]=!0;const j=q.endsWith(".css"),k=j?'[rel="stylesheet"]':"";if(!!b)for(let P=I.length-1;P>=0;P--){const G=I[P];if(G.href===q&&(!j||G.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${q}"]${k}`))return;const f=document.createElement("link");if(f.rel=j?"stylesheet":yb,j||(f.as="script"),f.crossOrigin="",f.href=q,E&&f.setAttribute("nonce",E),document.head.appendChild(f),j)return new Promise((P,G)=>{f.addEventListener("load",P),f.addEventListener("error",()=>G(new Error(`Unable to preload CSS for ${q}`)))})}))}function l(g){const I=new Event("vite:preloadError",{cancelable:!0});if(I.payload=g,window.dispatchEvent(I),!I.defaultPrevented)throw g}return i.then(g=>{for(const I of g||[])I.status==="rejected"&&l(I.reason);return a().catch(l)})},wb=n.lazy(()=>_(()=>import("./ClusterVisualizer-D_ileyyg.js"),__vite__mapDeps([0,1,2,3,4,5]),import.meta.url)),kb=n.lazy(()=>_(()=>import("./UR-platform-DFO5eSKd.js"),__vite__mapDeps([6,1,2,3,7,8,5]),import.meta.url)),zb=n.lazy(()=>_(()=>import("./heatmapper-CQ7wneDH.js"),__vite__mapDeps([9,1,2,3]),import.meta.url)),qb=n.lazy(()=>_(()=>import("./openstreetmap-wceuEFyo.js"),__vite__mapDeps([10,1,2,3,11,12,5]),import.meta.url)),Nb=n.lazy(()=>_(()=>import("./photogeo-DUM1KJgS.js"),__vite__mapDeps([13,1,2,3]),import.meta.url)),Mb=n.lazy(()=>_(()=>import("./photogeomap-BZHaPQu1.js"),__vite__mapDeps([14,1,2,3,11,12]),import.meta.url)),Sb=n.lazy(()=>_(()=>import("./clusteringevents-CQdjjSQo.js"),__vite__mapDeps([15,1,2,3,11,12,16]),import.meta.url));n.lazy(()=>_(()=>import("./litflow-DYT07kb5.js"),__vite__mapDeps([17,1,2,3]),import.meta.url));const Cb=n.lazy(()=>_(()=>import("./boxupcyberspace-CJjHQ5R1.js"),__vite__mapDeps([18,1,2,3]),import.meta.url)),Ib=n.lazy(()=>_(()=>import("./epsteinscret-mLJBzIXl.js"),__vite__mapDeps([19,1,2,3]),import.meta.url)),bc=n.lazy(()=>_(()=>import("./wechatgame-BR7BpSrd.js"),__vite__mapDeps([20,1,2,3]),import.meta.url)),Ab=n.lazy(()=>_(()=>import("./72HourGameJamDemo-BWPUgRvj.js"),__vite__mapDeps([21,1,2,3]),import.meta.url)),Db=n.lazy(()=>_(()=>import("./PeaceEliteHustCampusDemo-D-DWwJkq.js"),__vite__mapDeps([22,1,2,3]),import.meta.url)),Lb=n.lazy(()=>_(()=>import("./BMWMetaIslandDemo-7uu2MOZP.js"),__vite__mapDeps([23,1,2,3]),import.meta.url)),Pb=n.lazy(()=>_(()=>import("./riffle-RlweP75u.js"),__vite__mapDeps([24,1,2,3]),import.meta.url)),Eb=n.lazy(()=>_(()=>import("./genstyle-DuJ38oKx.js"),__vite__mapDeps([25,1,2,3]),import.meta.url)),Rb=n.lazy(()=>_(()=>import("./genshot-BoH35G7G.js"),__vite__mapDeps([26,1,2,3]),import.meta.url)),Tb=n.lazy(()=>_(()=>import("./dragon-diffusion-Dj4t-WNK.js"),__vite__mapDeps([27,1,2,3]),import.meta.url)),_b=n.lazy(()=>_(()=>import("./anyreal-DKKTpW8w.js"),__vite__mapDeps([28,1,2,3]),import.meta.url)),Gb=n.lazy(()=>_(()=>import("./previous-archi-work-CvNoz6hg.js"),__vite__mapDeps([29,1,2,3]),import.meta.url)),Bb=n.lazy(()=>_(()=>import("./hongqiqu-y3eLCJ-3.js"),__vite__mapDeps([30,1,2,3]),import.meta.url)),tc=c=>({"cluster-visualizer-3d":wb,"participatory-urban-regeneration-empowered-by-artificial-intelligence":kb,heatmapper:zb,"how-to-add-openstreetmap-in-website":qb,"how-to-get-sequential-events-from-photo-by-phone":Nb,"how-to-visualize-photo-geolocation-data-on-map":Mb,kittylovecarrots:bc,"how-to-build-game-in-wechat-mini-program":bc,"72-hour-game-jam":Ab,"peace-elite-hust-campus":Db,"bmw-meta-island":Lb,"how-to-conduct-time-geo-sequential-clustering-from-album":Sb,"box-up-my-stuff-in-cyberspace":Cb,"epstein-mystery-archive":Ib,"riffle-ai-game-generation-tool":Pb,genstyle:Eb,"genshot-AI-video-generation-tool":Rb,"dragon-diffussion":Tb,anyreal:_b,"previous-archi-work":Gb,hongqiqu:Bb})[c],ac={"how-to-add-openstreetmap-in-website":`// 1. 导入必要的库
