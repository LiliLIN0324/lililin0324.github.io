...
import{r as n,j as e,L as te,u as hc,R as lc,c as vc,d as mc,e as jc,f as F,H as yc,g as xc}from"./react-vendor-6zld6QO8.js";import{A as wc,m as Ze}from"./framer-DvVmTyA1.js";import{M as kc,r as zc,a as Nc}from"./markdown-q3GborES.js";import{b as Ye}from"./geo-Z02C8O4I.js";import{m as Ee}from"./maplibre-BedwuVXB.js";import{U as Je,L as qc,D as Cc,R as Mc,M as Ac,C as Lc,a as Sc,b as Ic,X as ec,c as cc}from"./icons-Dw2J7OpA.js";import"./vendor-C2tZ253J.js";(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))t(r);new MutationObserver(r=>{for(const l of r)if(l.type==="childList")for(const g of l.addedNodes)g.tagName==="LINK"&&g.rel==="modulepreload"&&t(g)}).observe(document,{childList:!0,subtree:!0});function u(r){const l={};return r.integrity&&(l.integrity=r.integrity),r.referrerPolicy&&(l.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?l.credentials="include":r.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function t(r){if(r.ep)return;r.ep=!0;const l=u(r);fetch(r.href,l)}})();function Dc({onFinish:c}){const[a,u]=n.useState(!0),t=n.useRef(null),r=n.useRef({x:-1e3,y:-1e3});return n.useEffect(()=>{const l=t.current;if(!l)return;const g=l.getContext("2d");if(!g)return;let S,w=[];const P=()=>{l.width=window.innerWidth,l.height=window.innerHeight},N=f=>{r.current={x:f.clientX,y:f.clientY}},j=f=>{f.touches.length>0&&(r.current={x:f.touches[0].clientX,y:f.touches[0].clientY})};window.addEventListener("resize",P),window.addEventListener("mousemove",N),window.addEventListener("touchmove",j),P();class k{constructor(){this.x=Math.random()*l.width,this.y=Math.random()*l.height,this.baseX=this.x,this.baseY=this.y,this.vx=(Math.random()-.5)*.5,this.vy=(Math.random()-.5)*.5,this.size=Math.random()*2+.5,this.density=Math.random()*30+1}update(){this.x+=this.vx,this.y+=this.vy,(this.x<0||this.x>l.width)&&(this.vx*=-1),(this.y<0||this.y>l.height)&&(this.vy*=-1);let E=r.current.x-this.x,_=r.current.y-this.y,B=Math.sqrt(E*E+_*_),D=E/B,J=_/B;const T=150;let C=(T-B)/T;B<T&&(this.x-=D*C*this.density*.5,this.y-=J*C*this.density*.5)}draw(){g&&(g.fillStyle="rgba(255, 255, 255, 0.8)",g.beginPath(),g.arc(this.x,this.y,this.size,0,Math.PI*2),g.fill())}}for(let f=0;f<120;f++)w.push(new k);const q=()=>{g.clearRect(0,0,l.width,l.height);for(let f=0;f<w.length;f++){w[f].update(),w[f].draw();for(let E=f+1;E<w.length;E++){const _=w[f].x-w[E].x,B=w[f].y-w[E].y,D=Math.sqrt(_*_+B*B);D<100&&(g.beginPath(),g.strokeStyle=`rgba(255, 255, 255, ${.2*(1-D/100)})`,g.lineWidth=.6,g.moveTo(w[f].x,w[f].y),g.lineTo(w[E].x,w[E].y),g.stroke())}}S=requestAnimationFrame(q)};return q(),()=>{cancelAnimationFrame(S),window.removeEventListener("resize",P),window.removeEventListener("mousemove",N),window.removeEventListener("touchmove",j)}},[]),e.jsx(wc,{onExitComplete:c,children:a&&e.jsxs(Ze.div,{onClick:()=>u(!1),initial:{opacity:1},exit:{opacity:0,filter:"blur(30px)",transition:{duration:1.2,ease:[.43,.13,.23,.96]}},className:"fixed inset-0 z-50 bg-black cursor-pointer overflow-hidden flex flex-col items-center justify-center",children:[e.jsx("canvas",{ref:t,className:"absolute inset-0 w-full h-full "}),e.jsx("div",{className:"relative z-20 text-center  select-none",children:e.jsx(Ze.p,{initial:{opacity:0,y:10},animate:{opacity:.7,y:0},transition:{duration:2},className:"text-white font-extralight text-lg md:text-sm uppercase tracking-[0.8em]",children:"LILI LIN's Space 林丽丽"})}),e.jsx(Ze.div,{initial:{opacity:0},animate:{opacity:.4},className:"absolute bottom-12 text-[10px] text-white font-light tracking-[0.5em] uppercase",children:"Touch the stars to enter"})]},"intro-screen")})}const Ec=`---
id: "00"
slug: "chicken-utopia"
...
## 本科研究\r
\r
### [neurotopia](#/planning/neurotopia)\r
\r
本科期间的一项城市设计研究，在韩国春川进行了分享。\r
\r
> 2023 · 韩国春川 · 国际学术交流分享\r
---\r
\r
\r
## 附图\r
...
`;function jt(c){const a=/^---\r?\n([\s\S]*?)\r?\n---/,u=c.match(a);let t={},r=c;if(u){const g=u[1];r=c.slice(u[0].length).trim();const S=g.split(/\r?\n/);let w=null,P=[];S.forEach(N=>{if(N.trim()==="")return;const k=N.indexOf(":");if(k!==-1&&!N.startsWith(" ")){w&&(t[w]=P.join(`
`)),w=N.slice(0,k).trim();let q=N.slice(k+1).trim();q==="|"?P=[]:q===""?P=[]:(t[w]=q,w=null,P=[])}else w&&N.startsWith("  ")&&P.push(N.slice(2))}),w&&P.length>0&&(t[w]=P.join(`
